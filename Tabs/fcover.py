...
##################################--Set & Calculate The FCOVER Parameters--##################################
metric = 'FCOVER'
...
def app():
    ...
    def get_plotting_data():
        ...
        for r in src_df.iterrows():
            ...
            xs, ys= main.get_avg_metric_vals_for_field_in_year(metric, field)
            print(len(xs), len(ys))
            plotting_data.append((xs,ys))
...
##################################--Plot Historic Avarages--##################################
    ...
    if display_historic_avgs == 'YES':
        ...
        fields_ids = main.get_list_of_fields()
        historic_avarage_plot = make_subplots(specs=[[{"secondary_y": True}]])
        for pair in plotting_data:
            historic_avarage_plot.add_trace(go.Scatter(
                x=pair[0],
                y=pair[1],
                name=f"Field {fields_ids[plotting_data.index(pair)]} data"), secondary_y=True)
    ...
    if display_historic_avgs == 'YES':
        ...
        if done:
            ...
            for row_fid in selected_f_ids:
                ...
                my_bar2.progress((selected_f_ids.index(row_fid)+ 1)/(len(src_df)))
